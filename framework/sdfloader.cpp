...
Scene SDFloader::load(std::string const& filename) {
  ...
  while (std::getline(file, curr_line)) {
    ...
    while(test >> curr_word) {
        ...
        if ("shape" == curr_word) {
          ...
          if ("composite" == curr_word) {
            ...
            for (auto const& it : parsed_children) {
              ...
              copy_temp.insert(std::make_pair(it_child->second->name(), it_child->second));
            }
      ...
      if ("transform" == curr_word) {
        std::string transform_name, transform_kind;
      ...
      if ("render" == curr_word) {
        unsigned out_x_res = 0, out_y_res = 0;
        std::string out_rendcam_name;
        std::string out_file_name;

        test >> out_rendcam_name >> out_file_name >> out_x_res >> out_y_res;
